...
public class ItemProcess {
	...
	public void ClickItemToItem(Player player, HashMap<String, Object> data, String Shop, String Key) {
		...
		int couu = Count > 0 ? (Count / (Float
				...
				: 0;
		list.add(new ElementSlider(
				TextFormat.WHITE + "您想兑换多少个 " + TextFormat.YELLOW
						+ ItemIDSunName.getIDByName(String.valueOf(data.get("ToBlockID"))) + TextFormat.WHITE + "?\n"
						+ TextFormat.WHITE + "您当前共有 " + TextFormat.GOLD + Count + TextFormat.WHITE + " 个"
						+ TextFormat.DARK_GREEN + ItemIDSunName.getIDByName(String.valueOf(data.get("BlockID"))) + "\n"
						...
						+ (Boolean.valueOf(String.valueOf(data.get("Astrict"))) ? ("\n" + TextFormat.WHITE + "剩余库存："
								+ TextFormat.RED + String.valueOf(data.get("ExpCount"))) : "")
